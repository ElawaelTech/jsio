...
export default function AdminEmpolyee() {
  ...
  async function GetEmployees() {
    try {
      ...
      const result = await fetch(ApiAdmin + "/Employees.php", {
        method: "POST",
      ...
      <div className="continer">
        <div className="row text-center ">
          <div className="col-12 text-center px-1">
            <h3 className="text-danger  text-center  p-2">الموظفين</h3>
                  ...
                  {Data.map((x, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className="text-center">{x.Mobil}</td>

                        <td className="text-center">{x.Job}</td>
                        <td className="text-center">{x.WorkZone}</td>
                        <td className="text-center">{x.DateJob}</td>
                        <td className="text-center">{x.Money}</td>
                        <td className="text-center">{x.Description}</td>
                        <td className="text-center">{x.Active}</td>
                      </tr>
                    );
                  })}
                </tbody>
