...
export default function AdminDeller() {
  ...
  //-------------------------------------------------------------
  async function GetDellers() {
    try {
      const post = {
        Action: "AdminDellers",
      };
      const result = await fetch(ApiAdmin + "/deller.php", {
        method: "POST",
  ...
  //-------------------------------------------------------------
  async function ActiveDeller(ID, Name, Statue) {
    var Statue_ = "";
    if (Statue == "disable") {
      Statue_ = "enable";
    }
      ...
      <div className="continer-floid0">
        <div className="row text-center ">
          <div className="col-12 text-center px-1">
            <h3 className="text-danger  text-center  p-2">المندوبين</h3>
              ...
              >
                ...
                <tbody>
                  {Data.map((x, index) => {
                    ...
                    if (x.Statue != "enable") {
                      classStatu = "text-danger text-white0";
                      Statu = "غير نشط";
                    }
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className="text-center">{x.CityWork}</td>
                        <td className="text-center">{x.Mony}</td>
                        <td className="text-center">{x.reviews}</td>
                        <td className="text-center px-1">
                          <label
                            onClick={() => ActiveDeller(x.ID, x.Name, x.Statue)}
...
//   'RkhsaType' => $RkhsaType,
//   'CarType' => $CarType,
//   'LawhatNumber' => $LawhatNumber,
//   'LawhatLeters' => $LawhatLeters,
